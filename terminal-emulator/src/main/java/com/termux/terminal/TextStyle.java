 ...
 * are stored for each cell in a terminal row in {@link TerminalRow#mStyle}.
 * </p>
 * <p>
 * The bit layout is:
 * </p>
...
public final class TextStyle {
   ...
   * characters defined as erasable. <p> This bit is set if DECSCA (Select
   * Character Protection Attribute) has been used to define the characters that
  ...
  /**
   * The 256 standard color entries and the three special (foreground,
   * background and cursor) ones.
   */
  public final static int NUM_INDEXED_COLORS = 259;

  /** Normal foreground and background colors and no effects. */
  final static long NORMAL =
      encode(COLOR_INDEX_FOREGROUND, COLOR_INDEX_BACKGROUND, 0);
  ...
  static long encode(int foreColor, int backColor, int effect) {
    ...
    if ((0xff000000 & foreColor) == 0xff000000) {
      // 24-bit color.
      result |= CHARACTER_ATTRIBUTE_TRUECOLOR_FOREGROUND |
                ((foreColor & 0x00ffffffL) << 40L);
    } else {
      // Indexed color.
    ...
    if ((0xff000000 & backColor) == 0xff000000) {
      // 24-bit color.
      result |= CHARACTER_ATTRIBUTE_TRUECOLOR_BACKGROUND |
                ((backColor & 0x00ffffffL) << 16L);
    } else {
      // Indexed color.
