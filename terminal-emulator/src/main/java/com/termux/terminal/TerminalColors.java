...
public final class TerminalColors {
  ...
  public static final TerminalColorScheme COLOR_SCHEME =
      new TerminalColorScheme();

  /**
   * The current terminal colors, which are normally set from the color theme,
   * but may be set dynamically with the OSC 4 control sequence.
   */
  ...
  /**
   * Reset a particular indexed color with the default color from the color
   * theme.
   */
  ...
  public void reset() {
    System.arraycopy(COLOR_SCHEME.mDefaultColors, 0, mCurrentColors, 0,
                     TextStyle.NUM_INDEXED_COLORS);
  }

  /**
   * Parse color according to
   * http://manpages.ubuntu.com/manpages/intrepid/man3/XQueryColor.3.html <p/>
   * Highest bit is set if successful, so return value is 0xFF${R}${G}${B}.
   * Return 0 if failed.
   */
  static int parse(String c) {
    try {
      ...
      if (c.charAt(0) == '#') {
        ...
        // rgb:<red>/<green>/<blue> where <red>, <green>, <blue> := h | hh | hhh
        // | hhhh. Scaled.
        skipInitial = 4;
        skipBetween = 1;
      } else {
      ...
      String rString =
          c.substring(currentPosition, currentPosition + componentLength);
