...
static int create_subprocess(JNIEnv *env, char const *cmd, char const *cwd,
                             char *const argv[], char **envp, int *pProcessId,
	...
	// Enable UTF-8 mode and disable flow control to prevent Ctrl+S from locking
	// up the display.
	struct termios tios;
	tcgetattr(ptm, &tios);
	tios.c_iflag |= IUTF8;
	...
	if (pid < 0) {
		...
		*pProcessId = (int)pid;
		...
		if (chdir(cwd) != 0) {
			char *error_message;
			// No need to free asprintf()-allocated memory since doing execvp() or
			// exit() below.
...
JNIEXPORT jint JNICALL Java_com_termux_terminal_JNI_createSubprocess(
	...
	if (size > 0) {
		...
		for (int i = 0; i < size; ++i) {
			jstring arg_java_string =
				(jstring)(*env)->GetObjectArrayElement(env, args, i);
			char const *arg_utf8 =
				(*env)->GetStringUTFChars(env, arg_java_string, NULL);
			if (!arg_utf8)
	...
	if (size > 0) {
		...
		for (int i = 0; i < size; ++i) {
			jstring env_java_string =
				(jstring)(*env)->GetObjectArrayElement(env, envVars, i);
	...
	int ptm = create_subprocess(env, cmd_utf8, cmd_cwd, argv, envp, &procId, rows,
	                            columns);
	...
	int *pProcId =
		(int *)(*env)->GetPrimitiveArrayCritical(env, processIdArray, NULL);
	if (!pProcId)
		return throw_runtime_exception(
			env,
...
JNIEXPORT void JNICALL Java_com_termux_terminal_JNI_setPtyWindowSize(
	JNIEnv *TERMUX_UNUSED(env), jclass TERMUX_UNUSED(clazz), jint fd, jint rows,
	jint cols) {
	struct winsize sz = {.ws_row = (unsigned short)rows,
...
JNIEXPORT jint JNICALL Java_com_termux_terminal_JNI_waitFor(
	...
	if (WIFEXITED(status)) {
		...
		// Should never happen - waitpid(2) says "One of the first three macros will
		// evaluate to a non-zero (true) value".
		return 0;
	}
}
