...
static {
	...
	TERMCAP_TO_KEYCODE.put("#2",
	                       KEYMOD_SHIFT | KEYCODE_MOVE_HOME); // Shifted home
	TERMCAP_TO_KEYCODE.put("#4", KEYMOD_SHIFT | KEYCODE_DPAD_LEFT);
	TERMCAP_TO_KEYCODE.put("*7",
	                       KEYMOD_SHIFT | KEYCODE_MOVE_END); // Shifted end key
	...
	TERMCAP_TO_KEYCODE.put("kd",
	                       KEYCODE_DPAD_DOWN); // terminfo=kcud1, down-arrow key
	...
	TERMCAP_TO_KEYCODE.put(
		"kB",
		KEYMOD_SHIFT | KEYCODE_TAB); // termcap=kB, terminfo=kcbt: Back-tab
	TERMCAP_TO_KEYCODE.put(
		"kD", KEYCODE_FORWARD_DEL); // terminfo=kdch1, delete-character key
	TERMCAP_TO_KEYCODE.put(
		"kDN",
		KEYMOD_SHIFT | KEYCODE_DPAD_DOWN); // non-standard shifted arrow down
	TERMCAP_TO_KEYCODE.put(
		"kF",
		KEYMOD_SHIFT | KEYCODE_DPAD_DOWN); // terminfo=kind, scroll-forward key
	...
	TERMCAP_TO_KEYCODE.put(
		"kR",
		KEYMOD_SHIFT | KEYCODE_DPAD_UP); // terminfo=kri, scroll-backward key
	TERMCAP_TO_KEYCODE.put(
		"kUP", KEYMOD_SHIFT | KEYCODE_DPAD_UP); // non-standard shifted up

	TERMCAP_TO_KEYCODE.put("@7", KEYCODE_MOVE_END);
	TERMCAP_TO_KEYCODE.put("@8", KEYCODE_NUMPAD_ENTER);
}

static String getCodeFromTermcap(String termcap,
...
public static String getCode(int keyCode, int keyMode, boolean cursorApp,
                             boolean keypadApplication) {
	switch (keyCode) {
	...
	case KEYCODE_MOVE_HOME:
		...
		// applications. On a Logitech k810 keyboard KEYCODE_MOVE_HOME is sent by
		// FN+LeftArrow.
	...
	// An xterm can send function keys F1 to F4 in two modes: vt100 compatible
	// or not. Because Vim may not know what the xterm is sending, both types of
	// keys are recognized. The same happens for the <Home> and <End> keys.
	// normal vt100 ~
	...
	case KEYCODE_F1:
		return (keyMode == 0) ? "\033OP"
		            : transformForModifiers("\033[1", keyMode, 'P');
	case KEYCODE_F2:
		return (keyMode == 0) ? "\033OQ"
		            : transformForModifiers("\033[1", keyMode, 'Q');
	case KEYCODE_F3:
		return (keyMode == 0) ? "\033OR"
		            : transformForModifiers("\033[1", keyMode, 'R');
	case KEYCODE_F4:
		return (keyMode == 0) ? "\033OS"
		            : transformForModifiers("\033[1", keyMode, 'S');
	case KEYCODE_F5:
	...
	case KEYCODE_SPACE:
		...
		// processing (which may e.g. cause a combining accent to be written):
	...
	case KEYCODE_NUMPAD_0:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 'p')
		               : "0";
	case KEYCODE_NUMPAD_1:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 'q')
		               : "1";
	case KEYCODE_NUMPAD_2:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 'r')
		               : "2";
	case KEYCODE_NUMPAD_3:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 's')
		               : "3";
	case KEYCODE_NUMPAD_4:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 't')
		               : "4";
	case KEYCODE_NUMPAD_5:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 'u')
		               : "5";
	case KEYCODE_NUMPAD_6:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 'v')
		               : "6";
	case KEYCODE_NUMPAD_7:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 'w')
		               : "7";
	case KEYCODE_NUMPAD_8:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 'x')
		               : "8";
	case KEYCODE_NUMPAD_9:
		return keypadApplication ? transformForModifiers("\033O", keyMode, 'y')
		               : "9";
...
private static String transformForModifiers(String start, int keymod,
                                            char lastChar) {
	int modifier;
	switch (keymod) {
	case KEYMOD_SHIFT:
