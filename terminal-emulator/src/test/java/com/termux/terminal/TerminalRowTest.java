...
/**
 * The properties of these code points are validated in {@link
 * #testStaticConstants()}.
 */
...
/**
 * Unicode Character 'MUSICAL SYMBOL G CLEF' (U+1D11E). Two java chars
 * required for this.
 */
static final int TWO_JAVA_CHARS_DISPLAY_WIDTH_ONE_1 = 0x1D11E;
/**
 * Unicode Character 'MUSICAL SYMBOL G CLEF OTTAVA ALTA' (U+1D11F). Two java
 * chars required for this.
 */
...
public void testUnicode() {
	...
	assertLineStartsWith(TWO_JAVA_CHARS_DISPLAY_WIDTH_ONE_1,
	                     TWO_JAVA_CHARS_DISPLAY_WIDTH_ONE_2, ' ');
...
public void testDoubleWidth() {
	...
	assertLineStartsWith(ONE_JAVA_CHAR_DISPLAY_WIDTH_TWO_1,
	                     ONE_JAVA_CHAR_DISPLAY_WIDTH_TWO_2);
...
public void testSurrogateCharsWithNormalDisplayWidth() {
	...
	for (int i = 0; i < 80; i++) {
		...
		assertEquals(i % 2 == 0 ? first : second,
...
public void testNormalCharsWithDoubleDisplayWidth() {
	...
	assertTrue(ONE_JAVA_CHAR_DISPLAY_WIDTH_TWO_1 !=
	           ONE_JAVA_CHAR_DISPLAY_WIDTH_TWO_2);
	assertEquals(1, Character.charCount(ONE_JAVA_CHAR_DISPLAY_WIDTH_TWO_1));
	assertEquals(1, Character.charCount(ONE_JAVA_CHAR_DISPLAY_WIDTH_TWO_2));
	assertEquals(2, WcWidth.width(ONE_JAVA_CHAR_DISPLAY_WIDTH_TWO_1));
...
public void testCrashATE145() {
	...
	int[] points =
		new int[] {0xC2541, 'a',    '8', 0x73EE, 0x009F, 0x881F, 0x8324,
		           0xD4C9,  0xFFFD, 'B', 0x009B, 0x61C9, 'Z'};
	// int[] expected = new int[] { TerminalEmulator.UNICODE_REPLACEMENT_CHAR,
	// 'a', '8', 0x73EE, 0x009F, 0x881F, 0x8324, 0xD4C9, 0xFFFD, 'B', 0x009B,
	// 0x61C9, 'Z' };
	int currentColumn = 0;
	...
	// charIndex++;
	// }
	// assertEquals("At index=" + i + ", charIndex=" + charIndex + ", char=" +
	// (char) thisPoint, points[i], thisPoint);
	// }
}
