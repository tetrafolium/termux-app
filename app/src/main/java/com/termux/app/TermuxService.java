...
/**
 * A service holding a list of terminal sessions, {@link #mTerminalSessions},
 ...
 * service may outlive the activity when the user or the system disposes of the
 * activity. In that case the user may restart {@link TermuxActivity} later to
 * yet again access the sessions. <p/> In order to keep both terminal sessions
 * and spawned processes (who may outlive the terminal sessions) alive as long
 * as wanted by the user this service is a foreground service, {@link
 * Service#startForeground(int, Notification)}. <p/> Optionally may hold a wake
 * and a wifi lock, in which case that is shown in the notification - see
 * {@link #buildNotification()}.
 */
public final class TermuxService
    extends Service implements SessionChangedCallback {
  ...
  private static final String ACTION_UNLOCK_WAKE =
      "com.termux.service_wake_unlock";
  /**
   * Intent action to launch a new terminal session. Executed from
   * TermuxWidgetProvider.
   */
  public static final String ACTION_EXECUTE = "com.termux.service_execute";

  public static final String EXTRA_ARGUMENTS = "com.termux.execute.arguments";

  public static final String EXTRA_CURRENT_WORKING_DIRECTORY =
      "com.termux.execute.cwd";
  private static final String EXTRA_EXECUTE_IN_BACKGROUND =
      "com.termux.execute.background";

  /**
   * This service is only bound from inside the same process and never uses
   * IPC.
   */
   ...
   * <p/>
   * Note that this list is observed by {@link TermuxActivity#mListViewAdapter},
  ...
  /**
   * Note that the service may often outlive the activity, so need to clear
   * this reference.
   */
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    if (ACTION_STOP_SERVICE.equals(action)) {
      ...
      String executablePath =
          (executableUri == null ? null : executableUri.getPath());

      String[] arguments =
          (executableUri == null ? null
                                 : intent.getStringArrayExtra(EXTRA_ARGUMENTS));
      String cwd = intent.getStringExtra(EXTRA_CURRENT_WORKING_DIRECTORY);

      if (intent.getBooleanExtra(EXTRA_EXECUTE_IN_BACKGROUND, false)) {
        BackgroundJob task =
            new BackgroundJob(cwd, executablePath, arguments, this);
        mBackgroundTasks.add(task);
        updateNotification();
      } else {
        boolean failsafe = intent.getBooleanExtra(
            TermuxActivity.TERMUX_FAILSAFE_SESSION_ACTION, false);
        TerminalSession newSession =
            createTermSession(executablePath, arguments, cwd, failsafe);

        // Transform executable path to session name, e.g.
        // "/bin/do-something.sh" => "do something.sh".
        if (executablePath != null) {
          int lastSlash = executablePath.lastIndexOf('/');
          String name = (lastSlash == -1)
                            ? executablePath
                            : executablePath.substring(lastSlash + 1);
          name = name.replace('-', ' ');
      ...
      Log.e(EmulatorDebug.LOG_TAG,
            "Unknown TermuxService action: '" + action + "'");
    }

    // If this service really do get killed, there is no point restarting it
    // automatically - let the user do on next start of {@link Term):
  ...
  void updateNotification() {
    if (mWakeLock == null && mTerminalSessions.isEmpty() &&
        mBackgroundTasks.isEmpty()) {
      // Exit if we are updating after the user disabled all locks with no
      // sessions or tasks running.
      stopSelf();
    } else {
  ...
  private Notification buildNotification() {
    ...
    // PendingIntent#getActivity(): "Note that the activity will be started
    ...
    // since it's using power, otherwise use a low priority
    builder.setPriority((wakeLockHeld) ? Notification.PRIORITY_HIGH
    ...
    String actionTitle =
        res.getString(wakeLockHeld ? R.string.notification_action_wake_unlock
                                   : R.string.notification_action_wake_lock);
    int actionIcon = wakeLockHeld ? android.R.drawable.ic_lock_idle_lock
  ...
  public void onDestroy() {
    ...
    if (termuxTmpDir.exists()) {
      try {
        ...
        Log.e(EmulatorDebug.LOG_TAG,
              "Error while removing file at " + termuxTmpDir.getAbsolutePath(),
              e);
      }
  ...
  public List<TerminalSession> getSessions() { return mTerminalSessions; }

  TerminalSession createTermSession(String executablePath, String[] arguments,
                                    String cwd, boolean failSafe) {
    ...
    String[] processArgs =
        BackgroundJob.setupProcessArgs(executablePath, arguments);
    executablePath = processArgs[0];
    int lastSlashIndex = executablePath.lastIndexOf('/');
    String processName =
        (isLoginShell ? "-" : "") +
        (lastSlashIndex == -1 ? executablePath
                              : executablePath.substring(lastSlashIndex + 1));
    ...
    TerminalSession session =
        new TerminalSession(executablePath, cwd, args, env, this);
  ...
  public int removeTermSession(TerminalSession sessionToRemove) {
    ...
    if (mTerminalSessions.isEmpty() && mWakeLock == null) {
      // Finish if there are no sessions left and the wake lock is not held,
      // otherwise keep the service alive if holding wake lock since there may
      // be daemon processes (e.g. sshd) running.
      stopSelf();
    } else {
      updateNotification();
    }
