 ...
 * visible on a logical screen and the scroll history. <p> See {@link
 * #externalToInternalRow(int)} for how to map from logical screen rows to array
 * indices.
 */
 ...
 *     transcript that holds lines that have scrolled off
...
public String getTranscriptText() {
	return getSelectedText(0, -getActiveTranscriptRows(), mColumns, mScreenRows)
	       .trim();
}
...
public String getSelectedText(int selX1, int selY1, int selX2, int selY2,
	...
	for (int row = selY1; row <= selY2; row++) {
		...
		int x2Index = (x2 < mColumns) ? lineObject.findStartOfColumn(x2)
		...
		if ((!joinBackLines || !rowLineWrap) && row < selY2 &&
		    row < mScreenRows - 1)
			builder.append('\n');
	}
 ...
 *
 * <pre>
 * - External coordinate system: -mActiveTranscriptRows to mScreenRows-1, with
 * the screen being 0..mScreenRows-1.
 * - Internal coordinate system: the mScreenRows lines starting at
 * mScreenFirstRow comprise the screen, while the mActiveTranscriptRows lines
 * ending at mScreenFirstRow-1 form the transcript (as a circular buffer).
 *
 * External ↔ Internal:
 *
 * [ ...                            ]     [ ... ] [ -mActiveTranscriptRows ]
 * [ mScreenFirstRow - mActiveTranscriptRows ] [ ... ]     [ ... ] [ 0
 * (visible screen starts here) ]  ↔  [ mScreenFirstRow ] [ ... ]     [ ... ]
...
/**
 * Resize the screen which this transcript backs. Currently, this only works
 * if the number of columns does not change or the rows expand (that is, it
 * only works when shrinking the number of rows).
 *
 * @param newColumns The number of columns the screen should have.
 * @param newRows    The number of rows the screen should have.
 * @param cursor     An int[2] containing the (column, row) cursor location.
 */
public void resize(int newColumns, int newRows, int newTotalRows,
                   int[] cursor, long currentStyle, boolean altScreen) {
	...
	if (newColumns == mColumns && newRows <= mTotalRows) {
		...
		if (shiftDownOfTopRow > 0 && shiftDownOfTopRow < mScreenRows) {
			...
			if (shiftDownOfTopRow != actualShift) {
				...
				for (int i = 0; i < actualShift - shiftDownOfTopRow; i++)
					allocateFullLineIfNecessary((mScreenFirstRow + mScreenRows + i) %
					                            mTotalRows)
					.clear(currentStyle);
		...
		// Blank lines should be skipped only if at end of transcript (just as is
		// done in the "fast" resize), so we keep track how many blank lines we
		// have skipped if we later on find a non-blank line.
		int skippedBlankLines = 0;
		for (int externalOldRow = -oldActiveTranscriptRows;
		     externalOldRow < oldScreenRows; externalOldRow++) {
			...
			if (cursorAtThisRow || oldLine.mLineWrap) {
				// Take the whole line, either because of cursor on it, or if line
				// wrapping.
				...
				for (int i = 0; i < oldLine.getSpaceUsed(); i++)
					// NEWLY INTRODUCED BUG! Should not index oldLine.mStyle with char
					// indices
					if (oldLine.mText[i] !=
			...
			for (int i = 0; i < lastNonSpaceIndex; i++) {
				...
				int outputColumn =
					currentOutputExternalColumn - offsetDueToCombiningChar;
				setChar(outputColumn, currentOutputExternalRow, codePoint,
				        styleAtCol);

				if (displayWidth > 0) {
					if (oldCursorRow == externalOldRow &&
					    oldCursorColumn == currentOldCol) {
			...
			// Old row has been copied. Check if we need to insert newline if old
			// line was not wrapping:
...
/**
 * Block copy lines and associated metadata from one location to another in
 * the circular buffer, taking wraparound into account.
 *
...
private void blockCopyLinesDown(int srcInternal, int len) {
	...
	TerminalRow lineToBeOverWritten =
		mLines[(srcInternal + start + 1) % totalRows];
	// Do the copy from bottom to top.
	for (int i = start; i >= 0; --i)
		mLines[(srcInternal + i + 1) % totalRows] =
			mLines[(srcInternal + i) % totalRows];
...
public void scrollDownOneLine(int topMargin, int bottomMargin, long style) {
	if (topMargin > bottomMargin - 1 || topMargin < 0 ||
	    bottomMargin > mScreenRows)
		throw new IllegalArgumentException("topMargin=" + topMargin +
		                                   ", bottomMargin=" + bottomMargin +
		                                   ", mScreenRows=" + mScreenRows);

	// Copy the fixed topMargin lines one line down so that they remain on
	// screen in same position:
	blockCopyLinesDown(mScreenFirstRow, topMargin);
	// Copy the fixed mScreenRows-bottomMargin lines one line down so that they
	// remain on screen in same position:
	blockCopyLinesDown(externalToInternalRow(bottomMargin),
	                   mScreenRows - bottomMargin);
 ...
 * within the bounds of the screen, or else an InvalidParameterException will
 * be thrown.
 *
...
public void blockCopy(int sx, int sy, int w, int h, int dx, int dy) {
	...
	for (int y = 0; y < h; y++) {
		...
		TerminalRow sourceRow =
			allocateFullLineIfNecessary(externalToInternalRow(sy + y2));
...
public void blockSet(int sx, int sy, int w, int h, int val, long style) {
	if (sx < 0 || sx + w > mColumns || sy < 0 || sy + h > mScreenRows) {
		throw new IllegalArgumentException(
			      "Illegal arguments! blockSet(" + sx + ", " + sy + ", " + w + ", " +
			      h + ", " + val + ", " + mColumns + ", " + mScreenRows + ")");
	}
	for (int y = 0; y < h; y++)
		for (int x = 0; x < w; x++)
...
public TerminalRow allocateFullLineIfNecessary(int row) {
	return (mLines[row] == null) ? (mLines[row] = new TerminalRow(mColumns, 0))
	                         : mLines[row];
}
 ...
 */
public void setOrClearEffect(int bits, boolean setOrClear, boolean reverse,
                             boolean rectangular, int leftMargin,
                             int rightMargin, int top, int left, int bottom,
                             int right) {
...
public void clearTranscript() {
	if (mScreenFirstRow < mActiveTranscriptRows) {
		Arrays.fill(mLines, mTotalRows + mScreenFirstRow - mActiveTranscriptRows,
		            mTotalRows, null);
		Arrays.fill(mLines, 0, mScreenFirstRow, null);
	} else {
		Arrays.fill(mLines, mScreenFirstRow - mActiveTranscriptRows,
		            mScreenFirstRow, null);
	}
