...
/**
 * The width of a single mono spaced character obtained by {@link
 * Paint#measureText(String)} on a single 'X'.
 */
 ...
 */
public final void render(TerminalEmulator mEmulator, Canvas canvas,
                         int topRow, int selectionY1, int selectionY2,
	...
	for (int row = topRow; row < endRow; row++) {
		...
		: charAtIndex;
			...
			final boolean insideCursor =
				(column >= selx1 && column <= selx2) ||
				(cursorX == column ||
				 (codePointWcWidth == 2 && cursorX == column + 1));
			final long style = lineObject.getStyle(column);

			// Check if the measured text width for this code point is not the same
			// as that expected by wcwidth(). This could happen for some fonts which
			// are not truly monospace, or for more exotic characters such as
			// smileys which android font renders as wide.
			// If this is detected, we draw this code point scaled to match what
			// wcwidth() expects.
			final float measuredCodePointWidth =
				(codePoint < asciiMeasures.length)
		? asciiMeasures[codePoint]
		: mTextPaint.measureText(line, currentCharIndex,
				         charsForCodePoint);
			final boolean fontWidthMismatch =
				Math.abs(measuredCodePointWidth / mFontWidth - codePointWcWidth) >
				0.01;

			if (style != lastRunStyle || insideCursor != lastRunInsideCursor ||
			    fontWidthMismatch || lastRunFontWidthMismatch) {
				if (column == 0) {
					// Skip first column as there is nothing to draw, just record the
					// current style.
				} else {
		    ...
		    : 0;
					drawTextRun(canvas, line, palette, heightOffset, lastRunStartColumn,
					            columnWidthSinceLastRun, lastRunStartIndex,
					            charsSinceLastRun, measuredWidthForRun, cursorColor,
					            cursorShape, lastRunStyle, reverseVideo);
				}
			...
			while (currentCharIndex < charsUsedInLine &&
				...
				// non-combining code point, instead of e.g. being considered inside
				// the cursor in the next run.
				currentCharIndex +=
...
private void drawTextRun(Canvas canvas, char[] text, int[] palette, float y,
                         ...
                         int cursor, int cursorStyle, long textStyle,
	...
	final boolean strikeThrough =
		(effect & TextStyle.CHARACTER_ATTRIBUTE_STRIKETHROUGH) != 0;
	...
	if (backColor != palette[TextStyle.COLOR_INDEX_BACKGROUND]) {
		...
		canvas.drawRect(left, y - mFontLineSpacingAndAscent + mFontAscent, right,
		                y, mTextPaint);
	}
