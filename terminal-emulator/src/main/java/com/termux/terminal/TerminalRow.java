...
final long[] mStyle;
/**
 * If this row might contain chars with width != 1, used for deactivating
 * fast path
 */
...
public void copyInterval(TerminalRow line, int sourceX1, int sourceX2,
                         int destinationX) {
	...
	final char[] sourceChars =
		(this == line) ? Arrays.copyOf(line.mText, line.mText.length)
		       : line.mText;
	int latestNonCombiningWidth = 0;
	for (int i = x1; i < x2; i++) {
		char sourceChar = sourceChars[i];
		int codePoint = Character.isHighSurrogate(sourceChar)
...
public int findStartOfColumn(int column) {
	...
	while (true) { // 0<2 1 < 2
		...
		int codePoint =
			isHigh ? Character.toCodePoint(c, mText[newCharIndex++]) : c;
			...
			if (currentColumn == column) {
				while (newCharIndex < mSpaceUsed) {
					...
					if (Character.isHighSurrogate(mText[newCharIndex])) {
						if (WcWidth.width(Character.toCodePoint(
									  mText[newCharIndex], mText[newCharIndex + 1])) <= 0) {
							newCharIndex += 2;
						} else {
							break;
						}
...
private boolean wideDisplayCharacterStartingAt(int column) {
	for (int currentCharIndex = 0, currentColumn = 0;
	     currentCharIndex < mSpaceUsed;) {
...
public void setChar(int columnToSet, int codePoint, long style) {
	...
	if (newIsCombining) {
		...
		// Check if we are overwriting the second half of a wide character
		// starting at the previous column:
		if (wasExtraColForWideChar)
			setChar(columnToSet - 1, ' ', style);
		// Check if we are overwriting the first half of a wide character starting
		// at the next column:
	...
	final int oldCodePointDisplayWidth =
		WcWidth.width(text, oldStartOfColumnIndex);
	...
	if (javaCharDifference > 0) {
		...
		if (mSpaceUsed + javaCharDifference > text.length) {
			...
			System.arraycopy(text, oldNextColumnIndex, newText, newNextColumnIndex,
			                 oldCharactersAfterColumn);
			mText = text = newText;
		} else {
			System.arraycopy(text, oldNextColumnIndex, text, newNextColumnIndex,
			                 oldCharactersAfterColumn);
		}
	...
	// noinspection ResultOfMethodCallIgnored - since we already now how many
	// java chars is used.
	Character.toChars(codePoint, text,
	                  oldStartOfColumnIndex +
	                  (newIsCombining ? oldCharactersUsedForColumn : 0));

	if (oldCodePointDisplayWidth == 2 && newCodePointDisplayWidth == 1) {
		// Replace second half of wide char with a space. Which mean that we
		// actually add a ' ' java character.
		if (mSpaceUsed + 1 > text.length) {
			char[] newText = new char[text.length + mColumns];
			System.arraycopy(text, 0, newText, 0, newNextColumnIndex);
			System.arraycopy(text, newNextColumnIndex, newText,
		...
		if (columnToSet == mColumns - 1) {
			throw new IllegalArgumentException(
				      "Cannot put wide character in last column");
		} else if (columnToSet == mColumns - 2) {
			// Truncate the line to the second part of this wide char:
			mSpaceUsed = (short)newNextColumnIndex;
		} else {
			// Overwrite the contents of the next column, which mean we actually
			// remove java characters. Due to the check at the beginning of this
			// method we know that we are not overwriting a wide char.
...
boolean isBlank() {
	for (int charIndex = 0, charLen = getSpaceUsed(); charIndex < charLen;
	     charIndex++)
		if (mText[charIndex] != ' ')
			return false;
	return true;
