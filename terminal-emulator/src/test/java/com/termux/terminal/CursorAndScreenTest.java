...
public class CursorAndScreenTest extends TerminalTestCase {
  ...
  public void testDeleteLinesKeepsStyles() {
    ...
    for (int row = 0; row < 5; row++) {
      for (int col = 0; col < 5; col++) {
        ...
        int wantedBackground =
            (row == 1 || row == 2) ? 99 : (row == 0 ? 2 : row);
        long s = getStyleAt(row, col);
  ...
  public void testDeleteCharacters() {
    ...
    withTerminalSized(5, 2)
        ...
        .assertLinesAre("ne   ", "     ");
    // "${CSI}${n}P, the delete characters (DCH) sequence should cap characters
    // to delete.
    withTerminalSized(5, 2)
        .enterString("nice")
    ...
    withTerminalSized(5, 2)
        ...
        .enterString("\033[2G\033[2P")
        .assertLinesAre("n\u0302e   ", "     ");
    // With wide 枝 char, checking that putting char at part replaces other with
    // whitespace:
    withTerminalSized(5, 2)
        .enterString("枝ce")
        .enterString("\033[Ga")
  ...
  public void testCursorForward() {
    ...
    withTerminalSized(6, 2)
        .enterString("A\033[CB\033[2CC")
        .assertLinesAre("A B  C", "      ");
    // If an attempt is made to move the cursor to the right of the right
    // margin, the cursor stops at the right margin:
    withTerminalSized(6, 2)
        .enterString("A\033[44CB")
        .assertLinesAre("A    B", "      ");
  ...
  public void testCursorUp() {
    ...
    withTerminalSized(3, 3)
        .enterString("ABCDEFG\033[AH")
        .assertLinesAre("ABC", "DHF", "G  ");
    withTerminalSized(3, 3)
        .enterString("ABCDEFG\033[2AH")
        .assertLinesAre("AHC", "DEF", "G  ");
    // If an attempt is made to move the cursor above the top margin, the cursor
    // stops at the top margin:
    withTerminalSized(3, 3)
        .enterString("ABCDEFG\033[44AH")
        .assertLinesAre("AHC", "DEF", "G  ");
    // Set top margin and validate that cursor does not go above it:
    withTerminalSized(3, 3)
        .enterString("\033[2rABCDEFG\033[44AH")
        .assertLinesAre("ABC", "DHF", "G  ");
  }
  ...
  public void testCursorDown() {
    ...
    withTerminalSized(3, 3)
        .enterString("AB\033[BC")
        .assertLinesAre("AB ", "  C", "   ");
    withTerminalSized(3, 3)
        .enterString("AB\033[2BC")
        .assertLinesAre("AB ", "   ", "  C");
    // If an attempt is made to move the cursor below the bottom margin, the
    // cursor stops at the bottom margin:
    withTerminalSized(3, 3)
        .enterString("AB\033[44BC")
        .assertLinesAre("AB ", "   ", "  C");
    // Set bottom margin and validate that cursor does not go above it:
    withTerminalSized(3, 3)
        .enterString("\033[1;2rAB\033[44BC")
        .assertLinesAre("AB ", "  C", "   ");
  }
  ...
  public void testReportCursorPosition() {
    ...
    for (int i = 0; i < 10; i++) {
      for (int j = 0; j < 10; j++) {
        enterString("\033[" + (i + 1) + ";" + (j + 1) +
                    "H"); // CUP cursor position.
        assertCursorAt(i, j);
        // Device Status Report (DSR):
        assertEnteringStringGivesResponse("\033[6n", "\033[" + (i + 1) + ";" +
                                                         (j + 1) + "R");
        // DECXCPR — Extended Cursor Position. Note that
        // http://www.vt100.net/docs/vt510-rm/DECXCPR says the response is
        // "${CSI}${LINE};${COLUMN};${PAGE}R" while xterm
        // (http://invisible-island.net/xterm/ctlseqs/ctlseqs.html) drops the
        // question mark. Expect xterm behaviour here.
        assertEnteringStringGivesResponse("\033[?6n", "\033[?" + (i + 1) + ";" +
                                                          (j + 1) + ";1R");
      }
    }
  ...
  /**
   * Test interactions between the cursor overflow bit and various escape
   * sequences. <p/> Adapted from hterm:
  ...
  public void testCursorSaveRestoreTextStyle() {
    ...
    // DEC save/restore
    withTerminalSized(4, 2).enterString(
        "\033[31;42;4m..\0337\033[36;47;24m\0338..");
    s = getStyleAt(0, 3);
    Assert.assertEquals(1, TextStyle.decodeForeColor(s));
    Assert.assertEquals(2, TextStyle.decodeBackColor(s));
    Assert.assertEquals(TextStyle.CHARACTER_ATTRIBUTE_UNDERLINE,
                        TextStyle.decodeEffect(s));

    // ANSI.SYS save/restore
    withTerminalSized(4, 2).enterString(
        "\033[31;42;4m..\033[s\033[36;47;24m\033[u..");
    s = getStyleAt(0, 3);
    Assert.assertEquals(1, TextStyle.decodeForeColor(s));
    Assert.assertEquals(2, TextStyle.decodeBackColor(s));
    Assert.assertEquals(TextStyle.CHARACTER_ATTRIBUTE_UNDERLINE,
