...
final class TermuxPreferences {
  ...
  @AsciiBellBehaviour int mBellBehaviour = BELL_VIBRATE;
  ...
  TermuxPreferences(Context context) {
    ...
    // This is a bit arbitrary and sub-optimal. We want to give a sensible
    // default for minimum font size to prevent invisible text due to zoom be
    // mistake:
    ...
    try {
      mFontSize = Integer.parseInt(
          prefs.getString(FONTSIZE_KEY, Integer.toString(defaultFontSize)));
  ...
  boolean toggleShowExtraKeys(Context context) {
    ...
    PreferenceManager.getDefaultSharedPreferences(context)
        .edit()
        .putBoolean(SHOW_EXTRA_KEYS_KEY, mShowExtraKeys)
        .apply();
  ...
  boolean isScreenAlwaysOn() { return mScreenAlwaysOn; }

  void setScreenAlwaysOn(Context context, boolean newValue) {
    mScreenAlwaysOn = newValue;
    PreferenceManager.getDefaultSharedPreferences(context)
        .edit()
        .putBoolean(SCREEN_ALWAYS_ON_KEY, newValue)
        .apply();
  }
  ...
  static void storeCurrentSession(Context context, TerminalSession session) {
    PreferenceManager.getDefaultSharedPreferences(context)
        .edit()
        .putString(TermuxPreferences.CURRENT_SESSION_KEY, session.mHandle)
        .apply();
  }
  ...
  static TerminalSession getCurrentSession(TermuxActivity context) {
    String sessionHandle =
        PreferenceManager.getDefaultSharedPreferences(context).getString(
            TermuxPreferences.CURRENT_SESSION_KEY, "");
    for (int i = 0, len = context.mTermService.getSessions().size(); i < len;
         i++) {
  ...
  void reloadFromProperties(Context context) {
    File propsFile =
        new File(TermuxService.HOME_PATH + "/.termux/termux.properties");
    if (!propsFile.exists())
      propsFile = new File(TermuxService.HOME_PATH +
    ...
    try {
      ...
      Toast
          .makeText(context,
                    "Could not open properties file termux.properties.",
                    Toast.LENGTH_LONG)
          .show();
    ...
    try {
      ...
      Toast
          .makeText(context,
                    "Could not load the extra-keys property from the config: " +
                        e.toString(),
                    Toast.LENGTH_LONG)
          .show();
      Log.e("termux", "Error loading props", e);
    ...
    shortcuts.clear();
    parseAction("shortcut.create-session", SHORTCUT_ACTION_CREATE_SESSION,
                props);
    ...
    parseAction("shortcut.previous-session", SHORTCUT_ACTION_PREVIOUS_SESSION,
                props);
    parseAction("shortcut.rename-session", SHORTCUT_ACTION_RENAME_SESSION,
                props);
  }
  ...
  private void parseAction(String name, int shortcutAction, Properties props) {
    ...
    if (!(parts.length == 2 && parts[0].trim().equals("ctrl")) ||
      ...
      Log.e("termux",
            "Keyboard shortcut '" + name + "' is not Ctrl+<something>");
      return;
    }
