...
public class DecSetTest extends TerminalTestCase {
  ...
  public void testShowHideCursor() {
    ...
    assertTrue("Initially the cursor should be visible",
               mTerminal.isShowingCursor());
    ...
    assertFalse(mTerminal.isShowingCursor());
    mTerminal.reset();
    assertTrue("Resetting the terminal should show the cursor",
               mTerminal.isShowingCursor());
  ...
  public void testBracketedPasteMode() {
    ...
    assertEquals(
        ...
        "a", mOutput.getOutputAndClear());

    enterString("\033[?2004h"); // Enable bracketed paste mode.
    mTerminal.paste("a");
    assertEquals("Pasting when in bracketed paste mode should be bracketed",
                 "\033[200~a\033[201~", mOutput.getOutputAndClear());
    ...
    assertEquals("Pasting an escape character should not input it",
                 "\033[200~abcd\033[201~", mOutput.getOutputAndClear());
    mTerminal.paste("\u0081ab\u0081cd\u009F");
    assertEquals("Pasting C1 control codes should not input it",
                 "\033[200~abcd\033[201~", mOutput.getOutputAndClear());

    mTerminal.reset();
    mTerminal.paste("a");
    assertEquals("Terminal reset() should disable bracketed paste mode", "a",
