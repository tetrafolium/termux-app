...
public void testMouseClick() throws Exception {
	...
	// When the client says that a click is outside (which could happen when
	// pixels are outside the terminal area, see
	// https://github.com/termux/termux-app/issues/501) the terminal sends a
	// click at the edge.
...
public void testReportTerminalSize() throws Exception {
	...
	for (int width = 3; width < 12; width++) {
		for (int height = 3; height < 12; height++) {
			...
			assertEnteringStringGivesResponse("\033[18t", "\033[8;" + height + ";" +
			                                  width + "t");
		}
	}
}
...
public void testSetCursorStyle() throws Exception {
	withTerminalSized(5, 5);
	assertEquals(TerminalEmulator.CURSOR_STYLE_BLOCK,
	             mTerminal.getCursorStyle());
	enterString("\033[3 q");
	assertEquals(TerminalEmulator.CURSOR_STYLE_UNDERLINE,
	             mTerminal.getCursorStyle());
	enterString("\033[5 q");
...
public void testBackgroundColorErase() {
	...
	for (int r = 0; r < rows; r++) {
		for (int c = 0; c < cols; c++) {
			...
			assertEquals(TextStyle.COLOR_INDEX_FOREGROUND,
			             TextStyle.decodeForeColor(style));
			assertEquals(TextStyle.COLOR_INDEX_BACKGROUND,
	...
	assertEquals(TextStyle.COLOR_INDEX_BACKGROUND,
	             TextStyle.decodeBackColor(getStyleAt(0, 2)));
}
...
public void testLineDrawing() {
	// 016 - shift out / G1. 017 - shift in / G0. "ESC ) 0" - use line drawing
	// for G1
	withTerminalSized(4, 2)
	.enterString("q\033)0q\016q\017q")
	.assertLinesAre("qq─q", "    ");
	// "\0337", saving cursor should save G0, G1 and invoked charset and "ESC 8"
	// should restore.
	withTerminalSized(4, 2)
...
public void testSoftTerminalReset() {
	...
	// "\033[?7l" is DECRST to disable wrap-around, and DECSTR ("\033[!p")
	// should reset it.
	withTerminalSized(3, 3)
	.enterString("\033[?7lABCD")
	.assertLinesAre("ABD", "   ", "   ");
