...
public class OperatingSystemControlTest extends TerminalTestCase {
  ...
  public void testSetTitle() throws Exception {
    ...
    expectedTitleChanges.add(
        new ChangedTitle("Hello, world", "Goodbye, world"));
    assertEquals(expectedTitleChanges, mOutput.titleChanges);

    enterString("\033]0;Goodbye, \u00F1 world\007");
    assertEquals("Goodbye, \uu00F1 world", mTerminal.getTitle());
    expectedTitleChanges.add(
        new ChangedTitle("Goodbye, world", "Goodbye, \uu00F1 world"));
    assertEquals(expectedTitleChanges, mOutput.titleChanges);
    ...
    expectedTitleChanges.add(
        new ChangedTitle("Goodbye, \uu00F1 world", "Updated"));
    assertEquals(expectedTitleChanges, mOutput.titleChanges);
  ...
  public void testSetColor() throws Exception {
    // "OSC 4; $INDEX; $COLORSPEC BEL" => Change color $INDEX to the color
    // specified by $COLORSPEC.
    withTerminalSized(4, 4).enterString("\033]4;5;#00FF00\007");
    assertEquals(Integer.toHexString(0xFF00FF00),
  ...
  public void testResetColor() throws Exception {
    ...
    for (int endType = 0; endType < 3; endType++) {
      ...
      for (int i = 0; i < 255; i++) {
        ...
        String rgbHex = String.format("%02x", r) + String.format("%02x", g) +
                        String.format("%02x", b);
  ...
  public void disabledTestSetClipboard() {
    ...
    enterString("\033]52;c;" +
                Base64.encodeToString("Hello, world".getBytes(), 0) + "\007");
  }
  ...
  public void testSettingDynamicColors() {
    // "${OSC}${DYNAMIC};${COLORSPEC}${BEL_OR_STRINGTERMINATOR}" => Change
    // ${DYNAMIC} color to the color specified by $COLORSPEC where: DYNAMIC=10:
    // Text foreground color. DYNAMIC=11: Text background color. DYNAMIC=12:
    // Text cursor color.
    ...
    enterString("\033]12;#00ABCD\007")
        .assertColor(TextStyle.COLOR_INDEX_CURSOR, 0xFF00ABCD);
    // Two special colors at once
    // ("Each successive parameter changes the next color in the list. The value
    // of P s tells the starting point in the list"):
  ...
  public void testReportSpecialColors() {
    // "${OSC}${DYNAMIC};?${BEL}" => Terminal responds with the control sequence
    // which would set the current color. Both xterm and libvte (gnome-terminal
    // and others) use the longest color representation, which means that the
    // response is "${OSC}rgb:RRRR/GGGG/BBBB"
    withTerminalSized(3, 3)
        .enterString("\033]10;#ABCD00\007")
        .assertColor(TextStyle.COLOR_INDEX_FOREGROUND, 0xFFABCD00);
    assertEnteringStringGivesResponse("\033]10;?\007",
                                      "\033]10;rgb:abab/cdcd/0000\007");
    // Same as above but with string terminator. xterm uses the same string
    // terminator in the response, which e.g. script posted at
    // http://superuser.com/questions/157563/programmatic-access-to-current-xterm-background-color
    // relies on:
    assertEnteringStringGivesResponse("\033]10;?\033\\",
                                      "\033]10;rgb:abab/cdcd/0000\033\\");
  }
}
