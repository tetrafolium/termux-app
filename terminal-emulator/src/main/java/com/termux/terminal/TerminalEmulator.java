 ...
 * explanation</li> <li>http://bjh21.me.uk/all-escapes/all-escapes.txt -
 * extensive!</li>
...
public final class TerminalEmulator {
  ...
  /**
   * Escape processing: Have seen an ESC character - proceed to {@link
   * #doEsc(int)}
   */
  ...
  /**
   * http://www.vt100.net/docs/vt510-rm/DECOM: "When DECOM is set, the home
   * cursor position is at the upper-left corner of the screen, within the
   * margins. The starting point for line numbers depends on the current top
   * margin setting. The cursor cannot move outside of the margins. When DECOM
   * is reset, the home cursor position is at the upper-left corner of the
   * screen. The starting point for line numbers is independent of the margins.
   ...
   * then graphic characters received when the cursor is at the right border of
   * the page appear at the beginning of the next line. Any text on the page
   * scrolls up if the cursor is at the end of the scrolling region. If the
   * DECAWM function is reset, then graphic characters received when the cursor
  ...
  /**
   * The normal screen buffer. Stores the characters that appear on the screen
   * of the emulated terminal.
   */
  private final TerminalBuffer mMainBuffer;
  /**
   * The alternate screen buffer, exactly as large as the display and contains
   * no additional saved lines (so that when the alternate screen buffer is
   * active, you cannot scroll back to view saved lines). <p> See
  ...
  private boolean mUseLineDrawingG0, mUseLineDrawingG1,
   ...
   * characters are inserted, pushing existing text to the right. Characters
   * moved past the right margin are lost.
   */
   ...
   * ranges from mTopMargin + 2 to mRows (Defines the first row after the
   * scrolling region). Left/right margin in [0, mColumns].
   */
  private int mTopMargin, mBottomMargin, mLeftMargin, mRightMargin;

  /**
   * If the next character to be emitted will be automatically wrapped to the
   * next line. Used to disambiguate the case where the cursor is positioned on
   * the last column (mColumns-1). When standing there, a written character will
   * be output in the last column, the cursor not moving but this flag will be
   * set. When outputting another character this will move to the next line.
   */
  private boolean mAboutToAutoWrap;

  /**
   * Current foreground and background colors. Can either be a color index in
   * [0,259] or a truecolor (24-bit) value. For a 24-bit value the top byte
   * (0xff000000) is set.
   *
   * @see TextStyle
   */
  ...
  /**
   * The number of scrolled lines since last calling {@link
   * #clearScrollCounter()}. Used for moving selection up along with the
   * scrolling text.
   */
  ...
  public boolean isAlternateBufferActive() { return mScreen == mAltBuffer; }

  /**
   * @param mouseButton one of the MOUSE_* constants of this class.
   */
  public void sendMouseEvent(int mouseButton, int column, int row,
                             boolean pressed) {
    if (column < 1)
      column = 1;
    if (column > mColumns)
    ...
    if (mouseButton == MOUSE_LEFT_BUTTON_MOVED &&
      ...
      mSession.write(String.format("\033[<%d;%d;%d" + (pressed ? 'M' : 'm'),
                                   mouseButton, column, row));
    } else {
  ...
  /**
   * {@link #CURSOR_STYLE_BAR}, {@link #CURSOR_STYLE_BLOCK} or {@link
      ...
      if ((byteToProcess & 0b11000000) == 0b10000000) {
        ...
        if (--mUtf8ToFollow == 0) {
          ...
          if (codePoint >= 0x80 && codePoint <= 0x9F) {
            // Sequence decoded to a C1 control character which we ignore. They
            // are not used nowadays and increases the risk of messing up the
            // terminal state on binary input. XTerm does not allow them in
            // utf-8: "It is not possible to use a C1 control obtained from
            // decoding the UTF-8 text" -
        ...
        // Not a UTF-8 continuation byte so replace the entire sequence up to
        // now with the replacement char:
        mUtf8Index = mUtf8ToFollow = 0;
        ...
        // with valid successor bytes (see Table 3-7), it must not consume the
        // successor bytes as part of the ill-formed subsequence
        // whenever those successor bytes themselves constitute part of a
  ...
  public void processCodePoint(int b) {
    ...
    case 0: // Null character (NUL, ^@). Do nothing.
      break;
    case 7: // Bell (BEL, ^G, \a). If in an OSC sequence, BEL may terminate a
            // string; otherwise signal bell.
    ...
    case 8: // Backspace (BS, ^H).
      ...
      break;
    case 9: // Horizontal tab (HT, \t) - move to next tab stop, but not past
            // edge of screen
      // XXX: Should perhaps use color if writing to new cells. Try with
      //       printf "\033[41m\tXX\033[0m\n"
      // The OSX Terminal.app colors the spaces from the tab red, but xterm does
      // not. Note that Terminal.app only colors on new cells, in e.g.
      //       printf "\033[41m\t\r\033[42m\tXX\033[0m\n"
      // the first cells are created with a red background, but when tabbing
      // over them again with a green background they are not overwritten.
    ...
    case 13: // Carriage return (CR, \r).
      ...
      break;
    case 14: // Shift Out (Ctrl-N, SO) → Switch to Alternate Character Set. This
             // invokes the G1 character set.
      mUseLineDrawingUsesG0 = false;
      break;
    case 15: // Shift In (Ctrl-O, SI) → Switch to Standard Character Set. This
    ...
    case 27: // ESC
      ...
      if (mEscapeState == ESC_P) {
        // XXX: Ignore escape when reading device control sequence, since it may
        // be part of string terminator.
        return;
    ...
    default:
      ...
      case ESC_POUND:
        doEscPound(b);
        break;
      case ESC_SELECT_LEFT_PAREN: // Designate G0 Character Set (ISO 2022,
                                  // VT100).
        mUseLineDrawingG0 = (b == '0');
        break;
      case ESC_SELECT_RIGHT_PAREN: // Designate G1 Character Set (ISO 2022,
                                   // VT100).
      ...
      case ESC_CSI_DOLLAR:
        ...
        case 'v': // ${CSI}${SRC_TOP}${SRC_LEFT}${SRC_BOTTOM}${SRC_RIGHT}${SRC_PAGE}${DST_TOP}${DST_LEFT}${DST_PAGE}$v"
          ...
          // http://vt100.net/docs/vt510-rm/DECCRA): "If Pbs is greater than
          // Pts, or Pls is greater than Prs, the terminal ignores DECCRA. The
          // coordinates of the rectangular area are affected by the setting of
          // origin mode (DECOM). DECCRA is not affected by the page margins.
          // The copied text takes on the line attributes of the destination
          // area. If the value of Pt, Pl, Pb, or Pr exceeds the width or height
          // of the active page, then the value is treated as the width or
          // height of that page. If the destination area is partially off the
          // page, then DECCRA clips the off-page data. DECCRA does not change
          // the active cursor position."
          int topSource =
              Math.min(getArg(0, 1, true) - 1 + effectiveTopMargin, mRows);
          int leftSource =
              Math.min(getArg(1, 1, true) - 1 + effectiveLeftMargin, mColumns);
          // Inclusive, so do not subtract one:
          int bottomSource = Math.min(
              Math.max(getArg(2, mRows, true) + effectiveTopMargin, topSource),
              mRows);
          int rightSource =
              Math.min(Math.max(getArg(3, mColumns, true) + effectiveLeftMargin,
                                leftSource),
                       mColumns);
          // int sourcePage = getArg(4, 1, true);
          int destionationTop =
              Math.min(getArg(5, 1, true) - 1 + effectiveTopMargin, mRows);
          int destinationLeft =
          ...
          int widthToCopy =
              Math.min(mColumns - destinationLeft, rightSource - leftSource);
          mScreen.blockCopy(leftSource, topSource, widthToCopy, heightToCopy,
                            destinationLeft, destionationTop);
          break;
        ...
        case 'z': // ${CSI}$${TOP}${LEFT}${BOTTOM}${RIGHT}$z"
          ...
          // "Pch can be any value from 32 to 126 or from 160 to 255. If Pch is
          // not in this range, then the terminal ignores the DECFRA command":
          if ((fillChar >= 32 && fillChar <= 126) ||
              (fillChar >= 160 && fillChar <= 255)) {
            // "If the value of Pt, Pl, Pb, or Pr exceeds the width or height of
            // the active page, the value is treated as the width or height of
            // that page."
            int top = Math.min(getArg(argIndex++, 1, true) + effectiveTopMargin,
                               effectiveBottomMargin + 1);
            int left =
                Math.min(getArg(argIndex++, 1, true) + effectiveLeftMargin,
                         effectiveRightMargin + 1);
            int bottom =
                Math.min(getArg(argIndex++, mRows, true) + effectiveTopMargin,
                         effectiveBottomMargin);
            int right =
                Math.min(getArg(argIndex, mColumns, true) + effectiveLeftMargin,
        ...
        case 't': // "${CSI}${TOP}${LEFT}${BOTTOM}${RIGHT}${ATTRIBUTES}$t"
          ...
          int top = Math.min(getArg(0, 1, true) - 1, effectiveBottomMargin) +
                    effectiveTopMargin;
          int left = Math.min(getArg(1, 1, true) - 1, effectiveRightMargin) +
                     effectiveLeftMargin;
          int bottom =
              Math.min(getArg(2, mRows, true) + 1, effectiveBottomMargin - 1) +
              effectiveTopMargin;
          int right = Math.min(getArg(3, mColumns, true) + 1,
                               effectiveRightMargin - 1) +
                      effectiveLeftMargin;
          if (mArgIndex >= 4) {
            ...
            for (int i = 4; i <= mArgIndex; i++) {
              ...
              case 0: // Attributes off (no bold, no underline, no blink,
                      // positive image).
              ...
              if (reverse && !setOrClear) {
                // Reverse attributes in rectangular area ignores non-(1,4,5,7)
                // bits.
              } else {
                mScreen.setOrClearEffect(
                    bits, setOrClear, reverse,
                    isDecsetInternalBitSet(
                        DECSET_BIT_RECTANGULAR_CHANGEATTRIBUTE),
                    effectiveLeftMargin, effectiveRightMargin, top, left,
                    bottom, right);
              }
            }
          } else {
      ...
      case ESC_CSI_SINGLE_QUOTE:
        if (b ==
          ...
          mScreen.blockCopy(mCursorCol, 0, columnsToMove, mRows,
                            mCursorCol + columnsToInsert, 0);
          blockClear(mCursorCol, 0, columnsToInsert, mRows);
        } else if (b == '~') { // Delete Ps Column(s) (default = 1) (DECDC),
                               // VT420 and up.
          int columnsAfterCursor = mRightMargin - mCursorCol;
          int columnsToDelete = Math.min(getArg0(1), columnsAfterCursor);
          int columnsToMove = columnsAfterCursor - columnsToDelete;
          mScreen.blockCopy(mCursorCol + columnsToDelete, 0, columnsToMove,
                            mRows, mCursorCol, 0);
      ...
      case ESC_CSI_QUESTIONMARK_ARG_DOLLAR:
        if (b == 'p') {
          ...
          if (mode == 47 || mode == 1047 || mode == 1049) {
            ...
            if (internalBit == -1) {
              value = isDecsetInternalBitSet(internalBit)
                          ? 1
                          : 2; // 1=set, 2=reset.
            } else {
          ...
          mSession.write(
              String.format(Locale.US, "\033[?%d;%d$y", mode, value));
        } else {
          unknownSequence(b);
        }
        break;
      ...
      case ESC_CSI_ARGS_ASTERIX:
        ...
        if (b == 'x' &&
          ...
          setDecsetinternalBit(DECSET_BIT_RECTANGULAR_CHANGEATTRIBUTE,
    ...
    case (byte)'\\': // End of ESC \ string Terminator
    {
      ...
      if (dcs.startsWith("$q")) {
        if (dcs.equals("$q\"p")) {
          ...
          // http://www.vt100.net/docs/vt510-rm/DECSCL:
        ...
        // Request Termcap/Terminfo String. The string following the "q" is a
        // list of names encoded in hexadecimal (2 digits per character)
        // separated by ; which correspond to termcap or terminfo key names. Two
        // special features are also recognized, which are not key names: Co for
        // termcap colors (or colors for terminfo colors), and TN for termcap
        // name (or name for terminfo name). xterm responds with DCS 1 + r P t
        // ST for valid requests, adding to P t an = , and the value of the
        // corresponding string that xterm would send, or DCS 0 + r P t ST for
        // invalid requests. The strings are encoded in hexadecimal (2 digits
        ...
        // Example: Request for ku is "ESC P + q 6 b 7 5 ESC \", where 6b7d=ku
        // in hexadecimal. Xterm response in normal cursor mode:
        // "<27> P 1 + r 6 b 7 5 = 1 B 5 B 4 1" where 0x1B 0x5B 0x41 = 27 91 65
        // = ESC [ A Xterm response in application cursor mode:
        ...
        for (String part : dcs.substring(2).split(";")) {
          if (part.length() % 2 == 0) {
            ...
            for (int i = 0; i < part.length(); i += 2) {
              char c =
                  (char)Long
                      .decode("0x" + part.charAt(i) + "" + part.charAt(i + 1))
                      .longValue();
            ...
            case "name":
              ...
              break;
            default:
              responseValue = KeyHandler.getCodeFromTermcap(
                  trans,
            ...
            if (responseValue == null) {
              ...
              for (int j = 0; j < responseValue.length(); j++) {
                hexEncoded.append(
                    String.format("%02X", (int)responseValue.charAt(j)));
              }
  ...
  private void doCsiQuestionMark(int b) {
    ...
    case 'n': // Device Status Report (DSR, DEC-specific).
      ...
      case 6:
        ...
        mSession.write(String.format(Locale.US, "\033[?%d;%d;1R",
      ...
      for (int i = 0; i <= mArgIndex; i++) {
        ...
        if (internalBit == -1) {
          ...
          if (b == 's') {
            ...
            doDecSetOrReset((mSavedDecSetFlags & internalBit) != 0,
                            externalBit);
          }
        }
      }
  ...
  public void doDecSetOrReset(boolean setting, int externalBit) {
    ...
    case 1: // Application Cursor Keys (DECCKM).
      break;
    case 3: // Set: 132 column mode (. Reset: 80 column mode. ANSI name:
            // DECCOLM.
      // We don't actually set/reset 132 cols, but we do want the side effects
      // (FIXME: Should only do this if the 95 DECSET bit (DECNCSM) is set, and
      // if changing value?): Sets the left, right, top and bottom scrolling
      // margins to their default positions, which is important for the "reset"
      // utility to really reset the terminal:
    ...
    case 1034: // Interpret "meta" key, sets eighth bit.
      break;
    case 1048: // Set: Save cursor as in DECSC. Reset: Restore cursor as in
               // DECRC.
      if (setting)
        saveCursor();
      else
    ...
    case 1047:
    case 1049: {
      // Set: Save cursor as in DECSC and use Alternate Screen Buffer, clearing
      // it first. Reset: Use Normal Screen Buffer and restore cursor as in
      // DECRC.
  ...
  private void doCsiBiggerThan(int b) {
    ...
    case 'c': // "${CSI}>c" or "${CSI}>c". Secondary Device Attributes (DA2).
      // Originally this was used for the terminal to respond with
      // "identification code, firmware version level, and hardware options"
      // (http://vt100.net/docs/vt510-rm/DA2), with the first "41" meaning the
      // VT420 terminal type. This is not used anymore, but the second version
      // level field has been changed by xterm to mean it's release number
      // ("patch numbers" listed at
      // http://invisible-island.net/xterm/xterm.log.html), and some
      // applications use it as a feature check:
      // * tmux used to have a "xterm won't reach version 500 for a while so set
      // that as the upper limit" check, and then check "xterm_version > 270" if
      // rectangular area operations such as DECCRA could be used.
      // * vim checks xterm version number >140 for "Request termcap/terminfo
      // string" functionality >276 for SGR mouse report. The third number is a
      // keyboard identifier not used nowadays.
    ...
    case 'm':
      ...
      // resources modifyKeyboard, modifyCursorKeys, modifyFunctionKeys and
      // modifyOtherKeys.
      ...
      // control, etc.) to handle special keyboard layouts (legacy and vt220).
      ...
      // modified. (1) allows modification of the numeric keypad (2) allows
      // modification of the editing keypad (4) allows modification of
      // function-keys, overrides use of Shift-modifier for UDK. (8) allows
      // modification of other special keys
      ...
      // - Set it to 2 to force the modifier to be the second parameter if it
      ...
      // resource values are similar to modifyCursorKeys: Set it to -1 to permit
      // the user to use shift- and control-modifiers to construct function-key
      ...
      // would otherwise be the first.
      // - Set it to 3 to mark the sequence with a ">" to hint that it is
      // private. If modifyFunctionKeys is zero, xterm uses Control- and
      // Shift-modifiers to allow the user to construct numbered function-keys
      // beyond the set provided by the keyboard: (Control) adds the value given
      // by the ctrlFKeys resource. (Shift) adds twice the value given by the
      // ctrlFKeys resource. (Control/Shift) adds three times the value given by
      // the ctrlFKeys resource.
      //
      // As a special case, legacy (when oldFunctionKeys is true) or vt220 (when
      // sunKeyboard is true) keyboards interpret only the Control-modifier when
      // constructing numbered function-keys. This is done to provide compatible
      ...
      // other keys (such as "2") when modified by Control-, Alt- or
      // Meta-modifiers. This feature does not apply to function keys and
      ...
      // cases, e.g., Control-Space to make a NUL. (2) enables this feature for
      // keys including the exceptions listed.
      Log.e(EmulatorDebug.LOG_TAG, "(ignored) CSI > MODIFY RESOURCE: " +
                                       getArg0(-1) + " to " + getArg1(-1));
      break;
    default:
      parseArg(b);
      break;
  ...
  private void doEsc(int b) {
    ...
    case ')':
      ...
      break;
    case '6': // Back index (http://www.vt100.net/docs/vt510-rm/DECBI). Move
              // left, insert blank column if start.
      if (mCursorCol > mLeftMargin) {
        mCursorCol--;
      } else {
        int rows = mBottomMargin - mTopMargin;
        mScreen.blockCopy(mLeftMargin, mTopMargin,
                          mRightMargin - mLeftMargin - 1, rows, mLeftMargin + 1,
                          mTopMargin);
    ...
    case '8': // DECRC restore cursor - http://www.vt100.net/docs/vt510-rm/DECRC
      restoreCursor();
      break;
    case '9': // Forward Index (http://www.vt100.net/docs/vt510-rm/DECFI). Move
              // right, insert blank column if end.
      if (mCursorCol < mRightMargin - 1) {
        mCursorCol++;
      } else {
        int rows = mBottomMargin - mTopMargin;
        mScreen.blockCopy(mLeftMargin + 1, mTopMargin,
                          mRightMargin - mLeftMargin - 1, rows, mLeftMargin,
                          mTopMargin);
    ...
    case 'c': // RIS - Reset to Initial State
              // (http://vt100.net/docs/vt510-rm/RIS).
      reset();
      blockClear(0, 0, mColumns, mRows);
      setCursorPosition(0, 0);
      break;
    ...
    case 'E': // Next line (http://www.vt100.net/docs/vt510-rm/NEL).
      setCursorCol(isDecsetInternalBitSet(DECSET_BIT_ORIGIN_MODE) ? mLeftMargin
                                                                  : 0);
      doLinefeed();
      break;
    ...
    case 'M': // "${ESC}M" - reverse index (RI).
      // http://www.vt100.net/docs/vt100-ug/chapter3.html: "Move the active
      // position to the same horizontal position on the preceding line. If the
  ...
  private void saveCursor() {
    SavedScreenState state =
        (mScreen == mMainBuffer) ? mSavedStateMain : mSavedStateAlt;
  ...
  private void restoreCursor() {
    ...
    mCurrentDecSetFlags =
        (mCurrentDecSetFlags & ~mask) | (state.mSavedDecFlags & mask);
  ...
  private void doCsi(int b) {
    ...
    case '@': {
      ...
      mScreen.blockCopy(mCursorCol, mCursorRow, charsToMove, 1,
                        mCursorCol + spacesToInsert, mCursorRow);
    ...
    case 'a': // "CSI${n}a" - Horizontal position relative (HPR). From
              // ISO-6428/ECMA-48.
    ...
    case 'f': // "${CSI}${ROW};${COLUMN}f" - Horizontal and Vertical Position
              // (HVP).
      setCursorPosition(getArg1(1) - 1, getArg0(1) - 1);
      break;
    case 'I': // Cursor Horizontal Forward Tabulation (CHT). Move the active
              // position n tabs forward.
      setCursorCol(nextTabStop(getArg0(1)));
      break;
    case 'J': // "${CSI}${0,1,2,3}J" - Erase in Display (ED)
      // ED ignores the scrolling margins.
      switch (getArg0(0)) {
      case 0: // Erase from the active position to the end of the screen,
              // inclusive (default).
        blockClear(mCursorCol, mCursorRow, mColumns - mCursorCol);
        blockClear(0, mCursorRow + 1, mColumns, mRows - (mCursorRow + 1));
        break;
      case 1: // Erase from start of the screen to the active position,
              // inclusive.
    ...
    case 'K': // "CSI{n}K" - Erase in line (EL).
      switch (getArg0(0)) {
      case 0: // Erase from the cursor to the end of the line, inclusive
              // (default)
        blockClear(mCursorCol, mCursorRow, mColumns - mCursorCol);
        break;
    ...
    {
      // http://www.vt100.net/docs/vt510-rm/DCH: "If ${N} is greater than the
      // number of characters between the cursor and the right margin, then DCH
      // only deletes the remaining characters. As characters are deleted, the
      // remaining characters between the cursor and right margin move to the
      // left. Character attributes move with the characters. The terminal adds
      ...
      mScreen.blockCopy(mCursorCol + cellsToDelete, mCursorRow, cellsToMove, 1,
    ...
    case 'T':
      if (mArgIndex == 0) {
        ...
        // http://vt100.net/docs/vt510-rm/SD: "N is the number of lines to move
        // the user window up in page memory. N new lines appear at the top of
        // the display. N old lines disappear at the bottom of the display. You
        // cannot pan past the top margin of the current page".
        ...
        mScreen.blockCopy(0, mTopMargin, mColumns,
                          linesBetweenTopAndBottomMargins - linesToScroll, 0,
      ...
      break;
    case 'X': // "${CSI}${N}X" - Erase ${N:=1} character(s) (ECH). FIXME: Clears
              // character attributes?
      mAboutToAutoWrap = false;
      mScreen.blockSet(mCursorCol, mCursorRow,
                       Math.min(getArg0(1), mColumns - mCursorCol), 1, ' ',
                       getStyle());
      break;
    case 'Z': // Cursor Backward Tabulation (CBT). Move the active position n
              // tabs backward.
    ...
    case 'c': // Primary Device Attributes
              // (http://www.vt100.net/docs/vt510-rm/DA1) if argument is missing
              // or zero.
      // The important part that may still be used by some (tmux stores this
      // value but does not currently use it) is the first response parameter
      // identifying the terminal service class, where we send 64 for "vt420".
    ...
    case 'l': // Reset Mode
      doSetMode(false);
      break;
    case 'm': // Esc [ Pn m - character attributes. (can have up to 16 numerical
              // arguments)
    ...
    case 'n': // Esc [ Pn n - ECMA-48 Status Report Commands
      ...
      case 6: // Cursor position report (CPR):
        ...
        // the cursor location.
        mSession.write(String.format(Locale.US, "\033[%d;%dR", mCursorRow + 1,
                                     mCursorCol + 1));
        break;
      default:
        break;
    ...
    case 'r': // "CSI${top};${bottom}r" - set top and bottom Margins (DECSTBM).
    {
      ...
      // The escape sequence numbers bottom 2..24, and so do we (because we use
      // a zero based numbering scheme, but we store the first line below the
      // bottom-most scrolling line. As a result, we adjust the top line by -1,
      // but we leave the bottom line alone. Also require that top + 2 <=
      // bottom.
      mTopMargin = Math.max(0, Math.min(getArg0(1) - 1, mRows - 2));
      mBottomMargin = Math.max(mTopMargin + 2, Math.min(getArg1(mRows), mRows));

      // DECSTBM moves the cursor to column 1, line 1 of the page respecting
      // origin mode.
      setCursorPosition(0, 0);
    } break;
    case 's':
      if (isDecsetInternalBitSet(DECSET_BIT_LEFTRIGHT_MARGIN_MODE)) {
        ...
        mRightMargin =
            Math.max(mLeftMargin + 1, Math.min(getArg1(mColumns), mColumns));
    ...
    case 't': // Window manipulation (from dtterm, as well as extensions)
      ...
      case 13: // Report xterm window position. Result is CSI 3 ; x ; y t
        ...
        break;
      case 14: // Report xterm window in pixels. Result is CSI 4 ; height ;
               // width t
        // We just report characters time 12 here.
        mSession.write(String.format(Locale.US, "\033[4;%d;%dt", mRows * 12,
                                     mColumns * 12));
        break;
      case 18: // Report the size of the text area in characters. Result is CSI
               // 8 ; height ; width t
        mSession.write(
            String.format(Locale.US, "\033[8;%d;%dt", mRows, mColumns));
        break;
      case 19: // Report the size of the screen in characters. Result is CSI 9 ;
        ...
        mSession.write(
            String.format(Locale.US, "\033[9;%d;%dt", mRows, mColumns));
        break;
      case 20: // Report xterm windows icon label. Result is OSC L label ST.
               // Disabled due to security concerns:
        mSession.write("\033]LIconLabel\033\\");
        break;
      case 21: // Report xterm windows title. Result is OSC l label ST. Disabled
    ...
    for (int i = 0; i <= mArgIndex; i++) {
      ...
      if (code == 0) { // reset
        ...
        if (firstArg == 2) {
          ...
          finishSequenceAndLogError("Invalid ISO-8613-3 SGR first argument: " +
                                    firstArg);
        }
        ...
        mBackColor = TextStyle.COLOR_INDEX_BACKGROUND;
                 ...
                 code <= 97) { // Bright foreground colors (aixterm codes).
        mForeColor = code - 90 + 8;
      } else if (code >= 100 &&
                 code <= 107) { // Bright background color (aixterm codes).
  ...
  private void doOscSetTextParameters(String bellOrStringTerminator) {
    ...
    for (int mOSCArgTokenizerIndex = 0;
      ...
      if (b == ';') {
        textParameter =
            mOSCOrDeviceControlArgs.substring(mOSCArgTokenizerIndex + 1);
        break;
      } else if (b >= '0' && b <= '9') {
    ...
    case 2: // Change window title to T.
      ...
      break;
    case 4:
      // P s = 4 ; c ; spec → Change Color Number c to the color specified by
      // spec. This can be a name or RGB specification as per XParseColor. Any
      // number of c name pairs may be given. The color numbers correspond to
      // the ANSI colors 0-7, their bright versions 8-15, and if supported, the
      // remainder of the 88-color or 256-color table. If a "?" is given rather
      // than a name or RGB specification, xterm replies with a control sequence
      // of the same form which can be used to set the corresponding color.
      // Because more than one pair of color number and specification can be
      // given in one control sequence, xterm can make more than one reply.
      int colorIndex = -1;
    ...
    case 12: // Set cursor color.
      ...
      for (int charIndex = 0;; charIndex++) {
        ...
        if (endOfInput || textParameter.charAt(charIndex) == ';') {
          try {
            String colorSpec =
                textParameter.substring(lastSemiIndex, charIndex);
            if ("?".equals(colorSpec)) {
              // Report current color in the same format xterm and
              // gnome-terminal does.
              ...
              mSession.write("\033]" + value +
                             ";rgb:" + String.format(Locale.US, "%04x", r) +
      ...
      break;
    case 52: // Manipulate Selection Data. Skip the optional first selection
             // parameter(s).
      int startIndex = textParameter.indexOf(";") + 1;
      try {
        String clipboardText =
            new String(Base64.decode(textParameter.substring(startIndex), 0),
      ...
      break;
    case 104:
      // "104;$c" → Reset Color Number $c. It is reset to the color specified by
      // the corresponding X resource. Any number of c parameters may be given.
      // These parameters correspond to the ANSI colors 0-7, their bright
      // versions 8-15, and if supported, the remainder of the 88-color or
      // 256-color table. If no parameters are given, the entire table will be
      // reset.
      if (textParameter.isEmpty()) {
        ...
        for (int charIndex = 0;; charIndex++) {
          ...
          if (endOfInput || textParameter.charAt(charIndex) == ';') {
            try {
              int colorToReset = Integer.parseInt(
  ...
  /**
   * NOTE: The parameters of this function respect the {@link
   * #DECSET_BIT_ORIGIN_MODE}. Use
  ...
  private void scrollDownOneLine() {
    ...
    if (mLeftMargin != 0 || mRightMargin != mColumns) {
      // Horizontal margin: Do not put anything into scroll history, just
      // non-margin part of screen up.
      mScreen.blockCopy(mLeftMargin, mTopMargin + 1, mRightMargin - mLeftMargin,
                        mBottomMargin - mTopMargin - 1, mLeftMargin,
                        mTopMargin);
      // .. and blank bottom row between margins:
      mScreen.blockSet(mLeftMargin, mBottomMargin - 1,
                       mRightMargin - mLeftMargin, 1, ' ', mEffect);
    } else {
  ...
  private void unimplementedSequence(int b) {
    logError("Unimplemented sequence char '" + (char)b + "' (U+" +
             String.format("%04x", b) + ")");
    finishSequence();
  }
  ...
  private void emitCodePoint(int codePoint) {
    ...
    if (autoWrap) {
      ...
      // The behaviour when a wide character is output with cursor in the last
      // column when autowrap is disabled is not obvious - it's ignored here.
      return;
    }
    ...
    if (mInsertMode && displayWidth > 0) {
      ...
      if (destCol < mRightMargin)
        mScreen.blockCopy(mCursorCol, mCursorRow, mRightMargin - destCol, 1,
                          destCol, mCursorRow);
    }

    int offsetDueToCombiningChar =
        ((displayWidth <= 0 && mCursorCol > 0 && !mAboutToAutoWrap) ? 1 : 0);
    mScreen.setChar(mCursorCol - offsetDueToCombiningChar, mCursorRow,
                    codePoint, getStyle());
  ...
  /**
   * Set the cursor mode, but limit it to margins if {@link
   * #DECSET_BIT_ORIGIN_MODE} is enabled.
   */
  ...
  /**
   * TODO: Better name, distinguished from {@link #setCursorPosition(int,
   * int)} by not regarding origin mode.
   */
  ...
  static final class SavedScreenState {
    /**
     * Saved state of the cursor position, Used to implement the save/restore
  ...
  public String toString() {
    return "TerminalEmulator[size=" + mScreen.mColumns + "x" +
        mScreen.mScreenRows + ", margins={" + mTopMargin + "," + mRightMargin +
