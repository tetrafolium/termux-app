...
public final class TerminalView extends View {
  ...
  /**
   * The currently displayed terminal session, whose emulator is {@link
   * #mEmulator}.
   */
  ...
  boolean mIsSelectingText = false, mIsDraggingLeftSelection,
          mInitialTextSelection;
  ...
  public TerminalView(Context context,
                      AttributeSet attributes) { // NO_UCD (unused code)
        ...
        context, new GestureAndScaleRecognizer.Listener() {
          ...
          public boolean onUp(MotionEvent e) {
            ...
            if (mEmulator != null && mEmulator.isMouseTrackingActive() &&
                !mIsSelectingText && !scrolledWithFinger) {
              // Quick event processing when mouse tracking is active - do not
              // wait for check of double tapping for zooming.
          ...
          @Override
          public boolean onScroll(MotionEvent e, float distanceX,
                                  float distanceY) {
            if (mEmulator == null || mIsSelectingText)
              return true;
            if (mEmulator.isMouseTrackingActive() &&
                e.isFromSource(InputDevice.SOURCE_MOUSE)) {
              // If moving with mouse pointer while pressing button, report that
              // instead of scroll. This means that we never report moving with
              // button press-events for touch input, since we cannot just start
              // sending these events without a starting press event, which we
              // do not do for touch input, only mouse in onTouchEvent().
              sendMouseEventCode(e, TerminalEmulator.MOUSE_LEFT_BUTTON_MOVED,
                                 true);
            } else {
          ...
          @Override
          public boolean onFling(final MotionEvent e2, float velocityX,
                                 float velocityY) {
            if (mEmulator == null || mIsSelectingText)
              return true;
            ...
            if (mouseTrackingAtStartOfFling) {
              mScroller.fling(0, 0, 0, -(int)(velocityY * SCALE), 0, 0,
                              -mEmulator.mRows / 2, mEmulator.mRows / 2);
            } else {
            ...
            post(new Runnable() {
              ...
              public void run() {
                ...
                int diff = mouseTrackingAtStartOfFling ? (newY - mLastY)
                                                       : (newY - mTopRow);
                doScroll(e2, diff);
                mLastY = newY;
                if (more)
  ...
  /**
   * @param onKeyListener Listener for all kinds of key events, both hardware
   *     and IME (which makes it different from that
   *                      available with {@link
  ...
  public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
    // Using InputType.NULL is the most correct input type and avoids issues
    // with other hacks.
    //
    ...
    // Note that IME_ACTION_NONE cannot be used as that makes it impossible to
    // input newlines using the on-screen keyboard on Android TV (see
    // https://github.com/termux/termux-app/issues/221).
    ...
    return new BaseInputConnection(this, true) {
      ...
      public boolean deleteSurroundingText(int leftLength, int rightLength) {
        if (LOG_KEY_EVENTS) {
          Log.i(EmulatorDebug.LOG_TAG, "IME: deleteSurroundingText(" +
        ...
        for (int i = 0; i < textLengthInChars; i++) {
          ...
          if (codePoint <= 31 && codePoint != 27) {
            if (codePoint == '\n') {
              ...
              // the enter key is pressed, instead of a key event like most
              // other keyboard apps. A terminal expects \r for the enter key
              // (although when icrnl is enabled this doesn't make a difference
              // - run 'stty -icrnl' to check the behaviour).
              codePoint = '\r';
            }
  ...
  void sendMouseEventCode(MotionEvent e, int button, boolean pressed) {
    ...
    int y = (int)((e.getY() - mRenderer.mFontLineSpacingAndAscent) /
    ...
    for (int i = 0; i < amount; i++) {
      if (mEmulator.isMouseTrackingActive()) {
        sendMouseEventCode(event,
                           up ? TerminalEmulator.MOUSE_WHEELUP_BUTTON
                              : TerminalEmulator.MOUSE_WHEELDOWN_BUTTON,
                           true);
      } else if (mEmulator.isAlternateBufferActive()) {
        // Send up and down key events for scrolling, which is what some
        // terminals do to make scroll work in e.g. less, which shifts to the
        // alt screen without mouse handling.
        handleKeyCode(
            up ? KeyEvent.KEYCODE_DPAD_UP : KeyEvent.KEYCODE_DPAD_DOWN, 0);
      } else {
        mTopRow = Math.min(
            0, Math.max(-(mEmulator.getScreen().getActiveTranscriptRows()),
                        mTopRow + (up ? -1 : 1)));
        if (!awakenScrollBars())
          invalidate();
      }
    ...
    if (mIsSelectingText) {
      ...
      if (ev.isButtonPressed(MotionEvent.BUTTON_SECONDARY)) {
        ...
        case MotionEvent.ACTION_UP:
          ...
          break;
        case MotionEvent.ACTION_MOVE:
          sendMouseEventCode(ev, TerminalEmulator.MOUSE_LEFT_BUTTON_MOVED,
                             true);
          break;
        }
        return true;
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    if (LOG_KEY_EVENTS)
      Log.i(EmulatorDebug.LOG_TAG, "onKeyDown(keyCode=" + keyCode +
    ...
    final boolean rightAltDownFromEvent =
    ...
    if (rightAltDownFromEvent) {
      ...
      // Use left alt to send to terminal (e.g. Left Alt+B to jump back a word),
      // so remove:
    ...
    if (LOG_KEY_EVENTS)
      Log.i(EmulatorDebug.LOG_TAG, "KeyEvent#getUnicodeChar(" +
                                       effectiveMetaState +
                                       ") returned: " + result);
    if (result == 0) {
      return false;
    ...
    if ((result & KeyCharacterMap.COMBINING_ACCENT) != 0) {
      ...
      if (mCombiningAccent != 0)
        inputCodePoint(mCombiningAccent, controlDownFromEvent,
                       leftAltDownFromEvent);
      mCombiningAccent = result & KeyCharacterMap.COMBINING_ACCENT_MASK;
    } else {
      if (mCombiningAccent != 0) {
        int combinedChar =
            KeyCharacterMap.getDeadChar(mCombiningAccent, result);
        if (combinedChar > 0)
          result = combinedChar;
        mCombiningAccent = 0;
      }
  ...
  void inputCodePoint(int codePoint, boolean controlDownFromEvent,
                      boolean leftAltDownFromEvent) {
    if (LOG_KEY_EVENTS) {
      Log.i(EmulatorDebug.LOG_TAG,
            "inputCodePoint(codePoint=" + codePoint +
                ", controlDownFromEvent=" + controlDownFromEvent +
    ...
    final boolean controlDown =
        controlDownFromEvent || mClient.readControlKey();
    ...
    if (controlDown) {
      if (codePoint >= 'a' && codePoint <= 'z') {
        ...
        // "Ctrl-/ sends 0x1f which is equivalent of Ctrl-_ since the days of
        // VT102"
        // -
    ...
    if (codePoint > -1) {
      // Work around bluetooth keyboards sending funny unicode characters
      // instead of the more normal ones from ASCII that terminal programs
      // expect - the desire to input the original characters should be low.
      switch (codePoint) {
      ...
      // If left alt, send escape before the code point to make e.g. Alt+B and
      // Alt+F work in readline:
  ...
  public boolean handleKeyCode(int keyCode, int keyMod) {
    ...
    String code =
        KeyHandler.getCode(keyCode, keyMod, term.isCursorKeysApplicationMode(),
   ...
   * were just added to the view hierarchy, you're called with the old values of
   * 0.
   */
  @Override
  ...
  public void updateSize() {
    ...
    int newRows =
        Math.max(4, (viewHeight - mRenderer.mFontLineSpacingAndAscent) /
                        mRenderer.mFontLineSpacing);
  ...
  protected void onDraw(Canvas canvas) {
    if (mEmulator == null) {
      ...
      mRenderer.render(mEmulator, canvas, mTopRow, mSelY1, mSelY2, mSelX1,
                       mSelX2);
      ...
      if (mIsSelectingText) {
        ...
        int right =
            Math.round((mSelX1)*mRenderer.mFontWidth) + gripHandleMargin;
        int top = (mSelY1 + 1 - mTopRow) * mRenderer.mFontLineSpacing +
                  mRenderer.mFontLineSpacingAndAscent;
        mLeftSelectionHandle.setBounds(
            right - gripHandleWidth, top, right,
            top + mLeftSelectionHandle.getIntrinsicHeight());
        mLeftSelectionHandle.draw(canvas);

        int left =
            Math.round((mSelX2 + 1) * mRenderer.mFontWidth) - gripHandleMargin;
        top = (mSelY2 + 1 - mTopRow) * mRenderer.mFontLineSpacing +
              mRenderer.mFontLineSpacingAndAscent;
        mRightSelectionHandle.setBounds(
            left, top, left + gripHandleWidth,
  ...
  public void toggleSelectingText(MotionEvent ev) {
    ...
    if (mIsSelectingText) {
      if (mLeftSelectionHandle == null) {
        mLeftSelectionHandle = (BitmapDrawable)getContext().getDrawable(
            R.drawable.text_select_handle_left_material);
        mRightSelectionHandle = (BitmapDrawable)getContext().getDrawable(
      ...
      int cy = (int)((ev.getY() + SELECT_TEXT_OFFSET_Y) /
                     mRenderer.mFontLineSpacing) +
               mTopRow;
      ...
      if (!" ".equals(screen.getSelectedText(mSelX1, mSelY1, mSelX1, mSelY1))) {
        ...
        while (mSelX1 > 0 && !"".equals(screen.getSelectedText(
                                 mSelX1 - 1, mSelY1, mSelX1 - 1, mSelY1))) {
          mSelX1--;
        }
        while (mSelX2 < mEmulator.mColumns - 1 &&
               !"".equals(screen.getSelectedText(mSelX2 + 1, mSelY1, mSelX2 + 1,
                                                 mSelY1))) {
          mSelX2++;
        }
      }
      ...
      final ActionMode.Callback callback = new ActionMode.Callback() {
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
          int show = MenuItem.SHOW_AS_ACTION_ALWAYS |
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
          ...
          case 2:
            ...
            if (clipData != null) {
              CharSequence paste =
                  clipData.getItemAt(0).coerceToText(getContext());
              if (!TextUtils.isEmpty(paste))
                mEmulator.paste(paste.toString());
            }
            break;
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        mActionMode = startActionMode(new ActionMode.Callback2() {
          ...
          @Override
          public void onGetContentRect(ActionMode mode, View view,
                                       Rect outRect) {
            int x1 = Math.round(mSelX1 * mRenderer.mFontWidth);
            int x2 = Math.round(mSelX2 * mRenderer.mFontWidth);
            int y1 =
                Math.round((mSelY1 - mTopRow) * mRenderer.mFontLineSpacing);
            int y2 =
                Math.round((mSelY2 + 1 - mTopRow) * mRenderer.mFontLineSpacing);
  ...
  private CharSequence getText() {
    return mEmulator.getScreen().getSelectedText(0, mTopRow, mEmulator.mColumns,
                                                 mTopRow + mEmulator.mRows);
  }
}
