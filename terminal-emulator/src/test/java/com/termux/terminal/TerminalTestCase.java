...
public abstract class TerminalTestCase extends TestCase {
  ...
  public void assertEnteringStringGivesResponse(String input,
  ...
  public void assertHistoryStartsWith(String... rows) {
    assertTrue("About to check " + rows.length + " lines, but only " +
  ...
  private static final class LineWrapper {
    ...
    public LineWrapper(TerminalRow line) { mLine = line; }
  ...
  protected TerminalTestCase assertInvariants() {
    ...
    for (int i = 0; i < lines.length; i++) {
      if (lines[i] == null)
        continue;
      assertTrue("Line exists at multiple places: " + i,
      ...
      for (int j = 0; j < usedChars; j++) {
        ...
        if (Character.isHighSurrogate(c)) {
          ...
          assertTrue("High surrogate without following low surrogate",
                     Character.isLowSurrogate(lowSurrogate));
          codePoint = Character.toCodePoint(c, lowSurrogate);
        } else {
          assertFalse("Low surrogate without preceding high surrogate",
                      Character.isLowSurrogate(c));
          codePoint = c;
        }
        assertFalse("Screen should never contain unassigned characters",
      ...
      assertEquals("Line whose width does not match screens. line=" +
                       new String(lines[i].mText, 0, lines[i].getSpaceUsed()),
                   screen.mColumns, currentColumn);
    }

    assertEquals("The alt buffer should have have no history",
                 mTerminal.mAltBuffer.mTotalRows,
                 mTerminal.mAltBuffer.mScreenRows);
    if (mTerminal.isAlternateBufferActive()) {
      assertEquals("The alt buffer should be the same size as the screen",
                   mTerminal.mRows, mTerminal.mAltBuffer.mTotalRows);
    }
  ...
  protected void assertLineIs(int line, String expected) {
    TerminalRow l = mTerminal.getScreen().allocateFullLineIfNecessary(
        mTerminal.getScreen().externalToInternalRow(line));
    char[] chars = l.mText;
    int textLen = l.getSpaceUsed();
    if (textLen != expected.length())
      fail("Expected '" + expected + "' (len=" + expected.length() +
           "), was='" + new String(chars, 0, textLen) + "' (len=" + textLen +
           ")");
    for (int i = 0; i < textLen; i++) {
      if (expected.charAt(i) != chars[i])
        fail("Expected '" + expected + "', was='" +
             new String(chars, 0, textLen) +
             "' - first different at index=" + i);
    }
  }
  ...
  protected TerminalTestCase assertLineStartsWith(int line, int... codePoints) {
    ...
    for (int i = 0; i < codePoints.length; i++) {
      ...
      if (Character.isHighSurrogate((char)lineCodePoint)) {
        lineCodePoint =
            Character.toCodePoint((char)lineCodePoint, chars[charIndex++]);
      }
      assertEquals("Differing a code point index=" + i, codePoints[i],
                   lineCodePoint);
    }
    return this;
  }
  ...
  public TerminalTestCase assertCursorAt(int row, int col) {
    ...
    if (!(row == actualRow && col == actualCol))
      fail("Expected cursor at (row,col)=(" + row + ", " + col + ") but was (" +
           actualRow + ", " + actualCol + ")");
    return this;
  }
  ...
  public static class EffectLine {
    ...
    public EffectLine(int[] styles) { this.styles = styles; }
  }

  protected EffectLine effectLine(int... bits) { return new EffectLine(bits); }
      ...
      for (int j = 0; j < line.length; j++) {
        ...
        if ((effectsAtCell & attributes) != attributes)
          fail("Line=" + i + ", column=" + j + ", expected " +
               describeStyle(attributes) + " set, was " +
      ...
      for (int j = 0; j < line.length; j++) {
        ...
        if (actualColor != expectedColor)
          fail("Line=" + i + ", column=" + j + ", expected color " +
               Integer.toHexString(expectedColor) + " set, was " +
  ...
  private static String describeStyle(int styleBits) {
    return "'" +
        ...
        ((styleBits & TextStyle.CHARACTER_ATTRIBUTE_INVERSE) != 0 ? ":INVERSE:"
                                                                  : "") +
        ((styleBits & TextStyle.CHARACTER_ATTRIBUTE_INVISIBLE) != 0
             ? ":INVISIBLE:"
             : "") +
        ((styleBits & TextStyle.CHARACTER_ATTRIBUTE_ITALIC) != 0 ? ":ITALIC:"
                                                                 : "") +
        ((styleBits & TextStyle.CHARACTER_ATTRIBUTE_PROTECTED) != 0
             ? ":PROTECTED:"
             : "") +
        ((styleBits & TextStyle.CHARACTER_ATTRIBUTE_STRIKETHROUGH) != 0
             ? ":STRIKETHROUGH:"
             : "") +
        ((styleBits & TextStyle.CHARACTER_ATTRIBUTE_UNDERLINE) != 0
             ? ":UNDERLINE:"
             : "") +
        "'";
  }
